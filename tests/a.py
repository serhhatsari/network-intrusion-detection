...
def add_pattern(pattern_entry, message_entry, pattern_listbox):
    ...
    if pattern and message:
        ...
        messagebox.showwarning("Input Error", "Both pattern and message fields are required.")
...
def create_gui(interface):
    ...
    signature_frame = ttk.Frame(root, width=window_width // 2, height=window_height // 2)
    signature_frame.pack(side=tk.TOP, padx=10, pady=10, fill=tk.BOTH, expand=True)
    ...
    # Add Pattern Section
    add_pattern_frame = ttk.Frame(root, width=window_width // 2, height=window_height // 2)
    add_pattern_frame.pack(side=tk.BOTTOM, padx=10, pady=10, fill=tk.BOTH, expand=True)

    add_pattern_label = ttk.Label(add_pattern_frame, text="Add New Pattern", font=("Helvetica", 16, "bold"))
    add_pattern_label.pack(pady=10)

    pattern_label = ttk.Label(add_pattern_frame, text="Pattern:")
    pattern_label.pack(pady=5)
    pattern_entry = ttk.Entry(add_pattern_frame, width=50)
    pattern_entry.pack(pady=5)

    message_label = ttk.Label(add_pattern_frame, text="Message:")
    message_label.pack(pady=5)
    message_entry = ttk.Entry(add_pattern_frame, width=50)
